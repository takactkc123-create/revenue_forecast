...
# ─── 特徴量列（04_model_train.py・05_predict_2026.py がモデルの入力に使う列）──────
# 2026-09-13変更: 「元データにある列」と「03_feature_eng.py が作る列」に分けて管理する。
#   - RAW_FEATURE_COLS      : 元データ（01の出力・実データのSQL抽出）にある列のうち、モデルに使うもの
#   - GENERATED_FEATURE_COLS: 03_feature_eng.py の preprocess() が作る列。ここに書いた列は03で必ず作ること
#     （03は preprocess() の最後で、この一覧と実際に作った列が一致するかを確認し、ずれていればエラーで止まる）
# ※ 除外列: 市町村_均等割 / 市町村_所得割 / 都道府県_均等割 / 都道府県_所得割
#   均等割・所得割の合計 = 年税額（目的変数）のため説明変数に使うと完全なリークになる
#   これらは03_feature_eng.pyでdrop済み
# ※ 仮ID・年度・年税額・年齢区分（文字列）・年齢・扶養人数・徴収区分 は、元データにあるがモデルには使わない
RAW_FEATURE_COLS = [
    # 属性
    "性別",
    # 収入（確定申告書第1表「収入金額等」）
    ...
    "社会保険料控除",
    "小規模企業共済等掛金控除",
    "生命保険料控除",         # 住民税上限7万円（一般・介護医療・個人年金の3区分合計）
    "地震保険料控除",         # 住民税上限2.5万円
    "雑損控除",
    "医療費控除",
    "障害者控除",
