...
- **外れ値への一定の耐性**：高所得者等の外れ値は個人単位の誤差（RMSE・WMAPE）を押し上げるが、税収予測の主指標である集計誤差率（予測合計と実績合計の差）への影響は、個人ごとの誤差のプラス・マイナスが相殺されるため小さくなりやすい。ただし目的変数（年税額）自体の分布は歪度が大きい右裾分布になりやすいため、`04`/`05`の`--target-transform`（none/log/winsorize/log_winsorize）で目的変数側の変換のみを選択できるようにしている。
...
| 指標 | TRAIN（2020〜2025 in-sample） | TEST（fold4: train=2020〜2024→test=2025） | 備考 | 位置づけ |
|---|---|---|---|---|
| 集計誤差率 | -0.00% | +0.01% | （予測合計−実績合計）÷実績合計。符号付きのため、個人ごとの誤差のプラス・マイナスが打ち消し合う | **主指標**：税収見込みの精度そのもの |
| WMAPE | 0.48% | 0.53% | 非課税者含む。Σ\|誤差\|÷Σ実績 ＝ 各人の誤差率を実績の税額で重み付けした平均。集計誤差率の上限（全員が同じ向きに外れた場合に一致） | 円ベース：税収のうち何円分を個人に誤って割り振ったか。高額納税者の外れは税額に比例して効く |
| MAPE | 1.61% | 1.40% | 課税者のみ（実績0円は分母にできないため除外）。各人の誤差率を同じ重みで平均 | 参考：人数ベースで、一般の納税者1人ひとりの当たり具合。少額納税者の外れで大きくなり、非課税者の誤予測は反映されない |
| MAE | 972円 | 1,232円 | | 特定の目的でのみ使用：1人あたり平均何円外れるかの把握。税収合計の精度判断には使わない |
| RMSE | 2,338円 | 4,545円 | | 特定の目的でのみ使用：高額納税者など大きな外れの荒れ具合の確認。外れ値に引っ張られるため、税収合計の精度判断には使わない |
| R² | 1.000 | 1.000 | | 参考 |
...
| 指標 | TRAIN | TEST | 備考 | 位置づけ |
|---|---|---|---|---|
| 集計誤差率 | （記入予定） | （記入予定） | （予測合計−実績合計）÷実績合計。符号付き | **主指標**：税収見込みの精度そのもの |
| WMAPE | （記入予定） | （記入予定） | 非課税者含む。各人の誤差率を実績の税額で重み付けした平均。集計誤差率の上限 | 円ベース：税収のうち何円分を個人に誤って割り振ったか。高額納税者の外れは税額に比例して効く |
| MAPE | （記入予定） | （記入予定） | 課税者のみ（実績0円は除外）。各人の誤差率を同じ重みで平均 | 参考：人数ベースで、一般の納税者1人ひとりの当たり具合。少額納税者の外れで大きくなり、非課税者の誤予測は反映されない |
| MAE | （記入予定） | （記入予定） | 円 | 特定の目的でのみ使用：1人あたり平均何円外れるかの把握。税収合計の精度判断には使わない |
| RMSE | （記入予定） | （記入予定） | 円 | 特定の目的でのみ使用：高額納税者など大きな外れの荒れ具合の確認。外れ値に引っ張られるため、税収合計の精度判断には使わない |
| R² | （記入予定） | （記入予定） | | 参考 |
...
新しい試行は既存ファイルを直接編集せず、`_V2`〜`_V5`として複製して変更点をファイル冒頭に記録している。

| バージョン | 見つかった課題 | 対応 |
|---|---|---|
| V1 | ダミー版をそのまま実データに流用したところ、2026年予測が実績の約3.7倍に膨らんだ。合計所得の計算対象に小計列と個別所得列、合計所得が重複して含まれ、同じ所得が3〜4重に加算されていたのが原因と判明 | 合計所得の対象を重複のない個別所得区分に限定、DBに存在するフィールドと生成するフィールドの切り分け |
| V2 | 2026年予測が約+10%の過大。翌年の所得控除を一部の控除だけで再計算しており、扶養・配偶者・障害者控除等がフラグしかもたないカラムであったため、課税標準額が学習時より高くなっていた | フラグ→控除金額に変換する特徴量を生成 |
| V3 | 集計誤差は約0.1%に収まったが、個人単位のブレが大きい。税額の歪度が100を超える極端な右裾分布だった | 目的変数を`log1p`変換して学習。逆変換時に過大予測（外れ値の影響で平均が過大に）したため不採用 |
| V4 | 対数変換以外（Winsorize、併用）も比較したい。変換ごとにファイルを複製すると修正漏れが起きる | 目的変数の変換を`--target-transform`/`--winsor-pct`で切り替え可能にし、ロジックを関数に集約。障害者・寡婦・配偶者・扶養控除の金額化もこの版で追加 |
| V5 | 集計精度は高いが、個人単位のRMSE・R²が悪い。高所得の外れ値が二乗誤差を支配していると推定 | LightGBMの損失関数を`--objective`で切り替え可能に（選択値は`05`が自動で引き継ぐ）。所得デシル別の予測分布と、非課税者を除いた課税者のみの合計・人数を追加 |
