...
| チェック項目 | 基準 | 結果 |
|---|---|---|
| 年度別合算の系統的バイアス | 平均誤差率が±1%以内 | 平均誤差率 +0.01% → クリア |
| フォールド間の安定性 | 誤差率の標準偏差が1.5%未満 | 標準偏差 0.01% → クリア |
| 過学習（訓練/テストMAE比） | 1.5倍未満=OK／2.0倍超=要見直し | 1,232円 ÷ 972円 ≈ 1.27倍 → OK |
| 外れ値（高所得デシル）依存度 | 最高所得デシルの誤差寄与率が30%超で要注意 | ダミー版無印`04_model_train.py`にはデシル別誤差分析が未実装のため未確認（実装済みなのはREAL版V4のみ） |
| 予測区間の実用性（`05`の95%信頼区間幅） | 業務上許容できる広さか | 個人単位 ±0.5万円／人。自治体合計では95%信頼区間 [238.25〜246.38] 億円（点予測242.22億円に対し概ね±1.7%） |
