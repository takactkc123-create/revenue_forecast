...
|---|---|---|---|
| WMAPE（税収予測の主指標） | 0.49% | 0.55% | 非課税者含む集計誤差率 |
| MAPE | 1.53% | 1.35% | 課税者のみ |
| MAE | 906円 | 1,072円 | |
| RMSE | 2,255円 | 3,802円 | |
| R² | 1.000 | 1.000 | |
...
| チェック項目 | 基準 | 結果 |
|---|---|---|
| 年度別合算の系統的バイアス | 平均誤差率が±1%以内 | 平均誤差率 +0.00% → クリア |
| フォールド間の安定性 | 誤差率の標準偏差が1.5%未満 | 標準偏差 0.01% → クリア |
| 過学習（訓練/テストMAE比） | 1.5倍未満=OK／2.0倍超=要見直し | 1,072円 ÷ 906円 ≈ 1.18倍 → OK |
| 外れ値（高所得デシル）依存度 | 最高所得デシルの誤差寄与率が30%超で要注意 | ダミー版無印`04_model_train.py`にはデシル別誤差分析が未実装のため未確認（実装済みなのはREAL版V4のみ） |
| 予測区間の実用性（`05`の95%信頼区間幅） | 業務上許容できる広さか | 個人単位 ±0.5万円／人。自治体合計では95%信頼区間 [201.21〜209.25] 億円（点予測205.12億円に対し概ね±2%） |

### 年度別合算精度（`04`最終モデルによる in-sample、参考値）

| 年度 | 実測（億円） | 予測（億円） | 誤差率 |
|---:|---:|---:|---:|
| 2020 | 177.05 | 177.03 | -0.01% |
| 2021 | 182.00 | 181.92 | -0.04% |
| 2022 | 180.18 | 180.22 | +0.02% |
| 2023 | 182.31 | 182.34 | +0.01% |
| 2024 | 186.73 | 186.74 | +0.00% |
| 2025 | 193.31 | 193.30 | -0.00% |

### 特徴量重要度 Top10（`04`）

`deduct_tax_furusato`（ふるさと納税控除額）＞ `taxable_income` ＞ `taxcredit_dividend_split` ＞ `deduct_tax_housing` ＞ `deduct_total` ＞ `prev_tax_amount` ＞ `income_total` ＞ `income_yoy_change` ＞ `deduct_rate` ＞ `income_property`
...
|---|---|
| 前年実績（2025年） | 193.31億円 |
| 2026年度 税収合計（最終・`06`のマクロ補正後） | 205.12億円（前年比 +6.11%） |
| 予測人数 | 98,800人 |
| 95%信頼区間 | [201.21 〜 209.25] 億円 |
| 給与収入上昇率（外挿、実績トレンド直近2年平均） | +6.80% |
| `06`トレンド補正・税制改正マクロ補正 | いずれも「適用する補正なし」（補正前後で205.12億円のまま） |

### 所見・特記事項

- ウォークフォワード検証（fold1〜4）はすべて誤差率±0.1%以内・個人MAPE 1.4〜1.7%で安定しており、バイアスや過学習の兆候は見られない。
- 予測値の押し上げ要因は主に給与収入トレンド（直近2年平均 +6.80%）であり、事業所得・年金収入はいずれもマイナス方向（-4.15%／-1.93%）に外挿されている。
...
| チェック項目 | 基準 | 結果 |
|---|---|---|
| 年度別合算の系統的バイアス | 平均誤差率が±1%以内 | （記入予定） |
| フォールド間の安定性 | 誤差率の標準偏差が1.5%未満 | （記入予定） |
| 過学習（訓練/テストMAE比） | 1.5倍未満=OK／2.0倍超=要見直し | （記入予定） |
| 外れ値（高所得デシル）依存度 | 最高所得デシルの誤差寄与率が30%超で要注意 | （記入予定） |
| 予測区間の実用性（`05`の95%信頼区間幅） | 業務上許容できる広さか | （記入予定） |
